...
const ThemeSwitch = styled(Switch)(({ theme }) => ({
    ...
    "& .MuiSwitch-thumb": {
        backgroundColor: theme.palette.mode === "dark" ? "#6B7280" : "#1F2937",
        width: 32,
        height: 32,
        "&:before": {
            content: "''",
...
const ThemeSwitcher = ({ className }: ThemeSwitcherProps) => {
    ...
    const onChange = (
        ...
        let cMode: ColorMode = "light";
        if (checked) cMode = "dark";
        else cMode = "light";

        setColorMode(cMode);
        setCookie(COLORMODE_CONSTANT, cMode, 10);
    };
    ...
    return (
        <Tooltip title={getTooltipData(colorMode)} placement="top">
            <ThemeSwitch
                className={className}
                onChange={onChange}
                value={colorMode}
